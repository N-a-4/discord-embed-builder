...
function buildSelectVarTS(varName, row){
  ...
  return [
    `{ // ${varName}`,
    `  const ${varName} = new ActionRowBuilder();`,
...
export function exportDiscordV2CodeTs(v2, varNames={}){
  ...
  // Iterate container components and chain calls
  for(const c of (container.components || [])){
    if(c.type === 'media'){
      ...
      if(media.length){
        lines.push(`    .addItems(`);
        // FIX: no stray ')' at end of this line
    ...
    if(c.type === 'section'){
      ...
      if(c.button){
        ...
        const emn = b?.emoji?.name ? emojiIdRef(b.emoji.name) : null;
        const emo = emn ? `.setEmoji({ id: ${emn} })` : '';
        lines.push(`  .addActionRowComponents(row => row.addComponents(new ButtonBuilder()${lab}${idOr}${sty}${emo}))`);
      }
      continue;
    }
    if(c.type === 'text'){
  ...
  // Build top-level rows: select first, then buttons (footer)
  const afterBlocks = [];
  ...
  for(const r of extras){
    ...
    if(r.type === 'action_row' && !haveButtonsRow){
      ...
      if(buttons.length){
        afterBlocks.push(
          `{ // ${buttonsName}`,
          `  const ${buttonsName} = new ActionRowBuilder();`,
          ...buttons.map(b => `  ${buttonsName}.addComponents(${buildButtonsInlineTS([b])});`),
          `}`
        );
        haveButtonsRow = true;
      }
  ...
  components: [${contName}${haveSelectRow ? `, ${selectName}`: ''}${haveButtonsRow ? `, ${buttonsName}`: ''}]
})`;
